...
module.exports = {
  parse: function (mixed, options) {
    ...
    return _.map(ws.Sheets, function (sheet, name) {
      return { name: name, data: XLSX.utils.sheet_to_json(sheet, { header: 1, raw: true, comment: true }) };
    });
  },
  parseandcomment: function (mixed, options) {
    var ws;
    if (typeof mixed === 'string') ws = XLSX.readFile(mixed, options);
    else ws = XLSX.read(mixed, options);
    return _.map(ws.Sheets, function (sheet, name) {
      var flag = { header: 1, raw: true, comment: true };
      return { name: name, data: XLSX.utils.sheet_to_json(sheet, flag), comment: XLSX.utils.sheet_to_comment(sheet, flag) };
    });
  },
