...
function sheet_to_comment(sheet, opts){
    var val, row, range, header = 0, offset = 1, r, hdr = [], isempty, R, C, c;
    var o = opts != null ? opts : {};
    var raw = o.raw;
    if(sheet == null || sheet["!ref"] == null) return [];
    range = o.range !== undefined ? o.range : sheet["!ref"];
    ...
    for (R = r.s.r + offset; R <= r.e.r; ++R) {
        ...
        for (C = r.s.c; C <= r.e.c; ++C) {
            val = sheet[cols[C] + rr];
            if(val === undefined || val.t === undefined|| val.c === undefined|| val.c.length === 0) continue;
            c = val.c[0].t;
            switch(val.t){
                case 'e': continue;
                case 's': break;
            ...
            if(c !== undefined) {
                row[hdr[C]] = raw ? c : format_cell(val,c);
                isempty = false;
            }
        }
        if(isempty === false || header === 1) out[outi++] = row;
...
var utils = {
    ...
    sheet_to_comment:sheet_to_comment,
